...
const GestReportPubliPage = () => {
    ...
    useEffect(() => {
        const cargarReportes = async () => {
            try {
                const res = await obtenerReportesPublicaciones();
                setReportes(res);
            } catch (error) {
    ...
    return (
        <div>
            ...
            <div className="container mx-auto px-4 py-6">
                <h1 className="text-2xl font-bold mb-4">Gestión de Reportes de Publicaciones</h1>
                            ...
                            {reportes.length === 0 ? (
                                <tr>
                                    <td colSpan="4" className="text-center py-4 text-gray-500">
                                        No hay reportes de publicaciones por revisar.
                                    </td>
                                </tr>
                            ) : (
                            reportes.map((r) => (
                                <tr key={r._id} className="text-center">
                                    ...
                                    <td className="py-2 px-4 border-b">
                                        {r.publicacion?.titulo || "Sin título"}
                                        <br />
                                        {r.publicacion?.imgURL && (
                                            <div className="flex justify-center mt-2">
                                                <img    
                                                    src={`http://localhost:4000/uploads/${r.publicacion.imgURL[0]}`}
                                                    alt="Publicación"
                                                    className="w-45 h-45 object-cover mt-2 rounded"
                                                />
                                            </div>
                                        )}
                                        <br />
                                        <span className="text-sm text-black">
                                            {r.publicacion?.descripcion || ""}
                                        </span>
                                    </td>
...
export default GestReportPubliPage;